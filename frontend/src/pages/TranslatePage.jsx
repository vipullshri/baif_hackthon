...
import { Type, FileAudio, FileVideo, Sparkles, AlertCircle, Wand2 } from 'lucide-react'
    ...
    try {
      if (mode === 'text') {
        ...
        if (created.status !== 'completed' && created.status !== 'failed') {
          stopRef.current = pollJob(created.id, (j) => {
            setJob(j)
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6 items-start">
      ...
      <div className="card p-6 space-y-5">
        ...
        {failed && (
          <div className="flex items-start gap-2 rounded-xl bg-red-500/10 border border-red-500/25 px-4 py-3 text-sm text-red-200">
            ...
            <span>{error || job?.error || 'Translation failed.'}</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-6 lg:sticky lg:top-24">
        {!job && !busy && <IdleHint />}
        {(processing || (busy && !completed)) && <ProgressTracker job={job || { progress: 5, status: 'processing' }} inputType={mode} />}
        {completed && <ResultPanel job={job} />}
      </div>
    </div>
  )
}
    ...
    <div className="card p-8 text-center relative overflow-hidden deva-watermark">
      <div className="relative z-10">
        ...
        <p className="mt-2 text-sand-300/60 max-w-sm mx-auto">
          Choose text, audio or video, pick your languages, and BhashaSetu will transcribe, translate,
