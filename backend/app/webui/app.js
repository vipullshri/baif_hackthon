...
(() => {
  ...
  const fileUrl = (id, kind, dl = false) => `${API}/jobs/${id}/file/${kind}${dl ? '?download=true' : ''}`;
  ...
  const STAGE_LABELS = {
    ...
    transcribing: 'Transcribing speech', translating: 'Translating', translated: 'Translated',
  ...
  function toast(msg, kind = 'ok') {
    ...
    setTimeout(() => { el.style.opacity = '0'; el.style.transform = 'translateX(20px)'; el.style.transition = 'all .3s'; setTimeout(() => el.remove(), 320); }, 3200);
  }
  ...
  function setMode(mode) {
    ...
    $('#mediaMode').classList.toggle('hidden', mode === 'text');
  ...
  function showResult(job) {
    ...
    card.innerHTML = `
      ...
      <div class="result-tabs" id="resTabs" style="margin-top:16px">
  ...
  function renderTab(job, tab) {
    ...
    if (tab === 'translation') {
      body.innerHTML = `<div class="text-out" id="trOut">${esc(job.translated_text)}</div>
        <div class="btn-row">
          <button class="btn btn-ghost btn-sm" onclick="App.copy(this, \`${job.translated_text.replace(/`/g, '\\`')}\`)">Copy</button>
      ...
      body.innerHTML = `<div class="text-out">${esc(job.source_text)}</div>
        <div class="btn-row"><button class="btn btn-ghost btn-sm" onclick="App.copy(this, \`${job.source_text.replace(/`/g, '\\`')}\`)">Copy</button>
  ...
  async function loadLibrary() {
    ...
    try {
      ...
      if (!data.items.length) { grid.innerHTML = `<div class="empty">No translations yet. Your processed text, audio and video will appear here.</div>`; return; }
      grid.innerHTML = data.items.map(jobCard).join('');
    } catch (e) { grid.innerHTML = `<div class="empty">Could not load library: ${e.message}</div>`; }
  }
  ...
  function jobCard(j) {
    ...
    return `<div class="card job-card" onclick="App.openJob('${j.id}')">
      <div class="spread"><h4>${esc(j.title || (j.input_type + ' translation'))}</h4></div>
  ...
  async function openJob(id) {
    try { const job = await http(`/jobs/${id}`); go('translate'); $('#progressCard').classList.add('hidden'); showResult(job); }
    catch (e) { toast(e.message, 'err'); }
  }
  ...
  function renderGlossary() {
    ...
    const rows = state.glossary.filter((e) =>
      !term || [e.en, e.hi, e.mr, e.category].some((v) => (v || '').toLowerCase().includes(term)));
    $('#gCount').textContent = `${rows.length}`;
    $('#gBody').innerHTML = rows.length ? rows.map((e) =>
      `<tr><td><span class="cat-tag">${esc(e.category)}</span></td><td>${esc(e.en)}</td><td>${esc(e.hi)}</td><td>${esc(e.mr)}</td>
        <td style="text-align:right"><button class="del-btn" title="Delete" onclick="App.delGlossary('${e.id}')">
  ...
  async function addGlossary() {
    const entry = { category: $('#gCat').value.trim() || 'general', en: $('#gEn').value.trim(), hi: $('#gHi').value.trim(), mr: $('#gMr').value.trim() };
    if (!entry.en || !entry.hi || !entry.mr) { toast('Fill English, Hindi and Marathi', 'err'); return; }
    try {
  ...
  async function delGlossary(id) {
    try { await http(`/glossary/${id}`, { method: 'DELETE' }); state.glossary = state.glossary.filter((e) => e.id !== id); renderGlossary(); }
    catch (e) { toast(e.message, 'err'); }
  }
  ...
  async function loadSystem() {
    try {
      ...
      $('#readyGrid').innerHTML = Object.entries(h.ready).map(([k, v]) =>
        `<div class="sys-item"><div class="k">${esc(k)}</div><div class="v"><span class="ready-dot ${v ? 'on' : 'off'}"></span>${v ? 'Ready' : 'Idle'}</div></div>`
      ).join('');
      $('#modeNote').innerHTML = h.models_enabled
        ...
        : `<div class="card-title" style="margin:0"><span class="dot"></span> Demo mode</div><p class="muted" style="margin:8px 0 0">Heavy ML models are disabled, so the full pipeline runs instantly with realistic mock output — perfect for testing the UI on standard hardware.</p>`;
