...
Video
All notebooks
Open Source Offline Translation Solution
BhashaSetu Installation Guide Text Conversion
BhashaSetu Installation and Packaging Guide
BhashaSetu Installation and Packaging Guide
BhashaSetu Documentation Transcription and Merge
Routes.py Code Transcription
Fixing GlossaryEntry.en AttributeError in FastAPI
Merging Split Code Images
BhashaSetu Windows Setup Script
Fixing SQLite Database Schema Error
OIDC vs. LDAP: Identity Management
GCP Lab: Secure Juice-Shop Network
Secure Juice-Shop Network Configuration
Image to Text and File Merging
React Components for Translation App
Image to Text and Code Transcription
API Routes and Schemas for Translation
Code and UI for Translation Pipeline
Image to Code Conversion and Merging
Image to Python Code Conversion
Code Extraction and Merging
Code Extraction and Merging
Image to Merged Code Conversion
JavaScript Code for Translation App
Marathi Audio to Text Conversion
Code Conversion and Merging
Image to Text and File Merging
Media Processing and Translation Pipeline
Code Extraction and Merging
API Routes and Job Cancellation
Pipeline Script Error Diagnosis
Handling Asyncio Connection Reset Errors
KB to MB Conversion
Hugging Face Symlink Warning Solutions
BhashaSetu Setup and Model Download
BhashaSetu Configuration and Launcher
GitHub Push Protection Blocked Secret
LED Backlit Dressing Mirror Search
Python Configuration and Settings Management
Configuration Files Transcription and Stitching
Conversation with Gemini
stitch and convert the image to text and provide the text in file

If space is tight, choose a larger drive (e.g. D:\) - the installer will ask you where to install.

2. Install Python 3.12
Go to https://www.python.org/downloads/ and download Python 3.12 (Windows installer, 64-bit).

Run the installer.

Important: on the first screen, tick "Add Python to PATH" before clicking Install Now.

Verify the install - open a new PowerShell window and run:
...
Go to https://nodejs.org and download the LTS version (18 or higher).
...
Add C:\ffmpeg\bin to PATH: open System Properties -> Environment Variables -> Path -> Edit -> New and add C:\ffmpeg\bin.
...
PowerShell
cd C:\
git clone [https://github.com/your-org/baif_hackathon.git](https://github.com/your-org/baif_hackathon.git)
cd baif_hackathon
...
PowerShell
$env:BHASHASETU_HF_TOKEN = "hf_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
...
BHASHASETU_HF_TOKEN=hf_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
If backend\.env does not exist yet, copy backend\.env.example to backend\.env first, then add the line.

Replace hf_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX with the actual token value from the email/communication.
...
Ask you which drive/folder to install into - press Enter to use the project folder, or type a path like D:\ for a separate drive.
...
Download all AI models from HuggingFace (~10-12 GB, first time only).

Build the React web frontend.

Save all configuration to backend\.env.
...
Complete the full install (§7) on a machine with internet.

Confirm the app works and models are downloaded.

Edit backend\.env and set:

BHASHASETU_OFFLINE=true
Copy the entire project folder (including translationService\ beside it) to the offline machine.

Run & .\scripts\install.ps1 on the offline machine - it will detect the existing install and start without downloading anything.
...
self-contained Windows application with PyInstaller. Two artifacts can
be produced:
...
Build the app folder first (.\scripts\build_exe.ps1), so dist\BhashaSetu\ exists.

Install Inno Setup (free): https://jrsoftware.org/isinfo.php - or winget install JRSoftware.InnoSetup.

Compile the bundled script:
...
Build on a machine with internet and ample disk - the result is multi-GB.

Use a build venv that also has the ML stack and the models pre-downloaded:
...
Edit installer\bhashasetu.spec (it has a commented FULL BUILD block showing exactly this):
...
Rebuild exactly as in §12.3. Expect a much larger artifact and a slower first run while models load.
...
Your markdown file is ready. I have stitched together the text from the images and preserved the formatting, code blocks, and tables as seen in the original document.
...
3. Add `C:fmpeg in` to PATH: open **System Properties** -> **Environment Variables** -> **Path** -> **Edit** -> **New** and add `C:fmpeg in`.
...
```powershell
cd C:git clone https://github.com/your-org/baif_hackathon.git
cd baif_hackathon
```
...
```
<install_base>	ranslationService\data├── outputs\       <- translated videos, dubbed audio, subtitle files (.srt, .vtt)
├── library\       <- completed jobs saved to the library (accessible from the Library tab)
├── uploads\       <- original uploaded files (temporary, cleaned up after processing)
├── logs\          <- application log files (see §10)
└── db\            <- SQLite database (glossary, job history)
...
- If you pressed Enter during install (default), it is the project folder itself:
  e.g. `C: aif_hackathon	ranslationService\data\`
- If you chose `D:\` during install, it is: `D:	ranslationService\data\`
- You can also check the value in `backend\.env`:

```powershell
Get-Content . ackend\.env | Select-String "BASE_DIR"
```
...
```
<install_base>	ranslationService\data\logs hashasetu.log
```
...
To tail the live log in PowerShell:
```powershell
Get-Content "<install_base>	ranslationService\data\logs hashasetu.log" -Wait -Tail 50
```
...
```
translationService├── .venv\                  Python virtual environment
├── models\                 downloaded AI model weights
└── data    ├── outputs\            translated videos, audio, subtitle files
    ├── library\            completed jobs (also accessible in the Library tab)
    ├── uploads\            temporary upload storage
    ├── logs\               bhashasetu.log (rotating)
    └── db\                 SQLite database
...
```
%LOCALAPPDATA%\BhashaSetu├── data\           SQLite db, uploads, outputs, media library, tmp
├── models\         downloaded models (Full build)
├── server-url.txt  the local URL the app is currently serving
└── bhashasetu.log  runtime log - check here first when troubleshooting
```
...
```powershell
# from the project root
.\scripts uild_exe.ps1 -Both       # build BOTH the portable .exe and the app folder
# ..or individually:
.\scripts uild_exe.ps1             # app folder only  -> dist\BhashaSetu.\scripts uild_exe.ps1 -OneFile    # portable file    -> dist\BhashaSetu.exe
.\scripts uild_exe.ps1 -Both -Clean # recreate the build venv first
```

The script creates an **isolated build venv** (core deps + PyInstaller only - no ML stack), runs the
bundled spec at `installer hashasetu.spec`, and writes the result to `dist\`.
...
# app folder build:
python -m PyInstaller --noconfirm --distpath ..\dist --workpath .. uild\pyinstaller ..\installer hashasetu.spec

# portable single-file build:
$env:BHASHASETU_ONEFILE = "1"
python -m PyInstaller --noconfirm --distpath ..\dist --workpath .. uild\pyinstaller-onefile ..\installer hashasetu.spec
```
...
1. Build the **app folder** first (`.\scripts uild_exe.ps1`), so `dist\BhashaSetu\` exists.
...
3. Compile the bundled script:
   ```powershell
   & "C:\Program Files (x86)\Inno Setup 6\ISCC.exe" installer hashasetu.iss
   ```
   The installer is written to `dist\BhashaSetu-Setup-1.0.0.exe`. It installs **per-user by default**
   (no admin required) and offers to remove the data folder on uninstall.
...
2. Use a build venv that also has the ML stack and the models pre-downloaded:
   ...
   pip install -r requirements-ml.txt       # PyTorch (CPU), transformers, faster-whisper, …
   python ..\scripts\download_models.py     # pre-fetch models into backend\models   ```
3. Edit `installer hashasetu.spec` (it has a commented **FULL BUILD** block showing exactly this):
   - Set `excludes = []` (stop excluding the ML libraries).
   - Uncomment the `collect_all(...)` loop so `torch`, `transformers`, `ctranslate2`,
     `sentencepiece`, `tokenizers`, `soundfile` and `numpy` are fully bundled.
   - Drop `ffmpeg.exe` at `installer infmpeg.exe` - the spec auto-bundles `installer in\` to
     `bin\`, and the launcher adds it to `PATH` at runtime.
   - Optionally bundle the downloaded `models\` folder, or let the app fetch them on first run into
     `%LOCALAPPDATA%\BhashaSetu\models\`.
...
| Symptom | Fix |
| ------- | --- |
| Window opens but the browser shows an error / nothing | Give it a few seconds on first launch; then read `%LOCALAPPDATA%\BhashaSetu hashasetu.log`. |
| App exits immediately | Check `%LOCALAPPDATA%\BhashaSetu\last-crash.log` for the traceback. |
| "Windows protected your PC" (SmartScreen) | The exe is unsigned. Click **More info -> Run anyway**, or code-sign it for wide distribution. |
| Port 8000 busy | The launcher auto-selects the next free port; the actual URL is in `server-url.txt`. |
| Antivirus flags the onefile exe | A known PyInstaller false-positive. Prefer the **app folder** build, or add an exclusion / code-sign it. |
INSTALLATION.md
Displaying INSTALLATION.md.